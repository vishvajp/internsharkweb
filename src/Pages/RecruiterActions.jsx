...
const RecruiterActions = () => {
  ...
  //node backend data
  useEffect(() => {
    const getrecruiteraction = async () => {
      try {
        const response = await getappliedjobs();
        setAppliedJobList(response);
        console.log("//////////////////", response);
      } catch (error) {
        console.log("Error while fetch application detail", error);
        setAppliedJobList([]);
      }
    };
    getrecruiteraction();
      ...
      <div className="container py-4">
        {!appliedJobList ? (
          ...
          <div className="d-flex flex-column justify-content-center align-items-center py-5">
            <p className="text-muted">No RecruiterActions found.</p>
          </div>
        ) : (
          (() => {
            // 🔹 Filter the jobs by status
            const filteredJobs = appliedJobList.filter(
              (item) =>
            ...
            // 🔹 If no jobs match those statuses
            ...
            // 🔹 Otherwise, render filtered jobs
            return (
              <div className="row">
                      ...
                      >
                        ...
                        >
                          {item.job.intershipdescription
                            ...
                            .slice(0, 20)
                            .join(" ") +
                            (item.job.intershipdescription?.split(" ").length >
                            20
                              ? "..."
                              : "")}
                        </div>
